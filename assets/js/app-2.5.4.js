// i18n: vlkT é definido por assets/js/i18n/i18n.js (carregado antes deste arquivo)
if (!window.vlkT) { window.vlkT = function (k) { return k; }; }
/*
...
(function(OpenSpeedTest) {
  ...
  var openSpeedtestShow = function() {
    ...
    this.oDoMidSpeed = _("oDoMidSpeed");
  ...
  openSpeedtestShow.prototype.adjustScale = function(estimatedSpeed) {
    var ceilings = [100, 200, 500, 1000, 2000, 5000, 10000, 25000, 40000, 100000];
    var maxVal = ceilings[ceilings.length - 1];
    for (var i = 0; i < ceilings.length; i++) {
      if (estimatedSpeed * 1.5 <= ceilings[i]) {
        maxVal = ceilings[i];
        break;
      }
    }
    ...
    if (maxVal <= 100) {
      newScale = [
        ...
        {degree:460, value:1},
        {degree:337, value:10},
        {degree:180, value:Math.max(1, Math.round(maxVal * 0.5))},
        {degree:0, value:maxVal}
      ];
    } else if (maxVal <= 200) {
      newScale = [
        {degree:680, value:0},
        ...
        {degree:110, value:Math.round(maxVal * 0.75)},
        {degree:0, value:maxVal}
      ];
    } else {
      newScale = [
        ...
        {degree:337, value:10},
        {degree:220, value:100},
        {degree:110, value:Math.round(maxVal * 0.5)},
        {degree:0, value:maxVal}
      ];
    }
    this.scale = newScale;
    var label = maxVal >= 1000 ? (maxVal / 1000).toFixed(0) + "G+" : maxVal + "+";
    this.oDoTopSpeed.el.textContent = label;
    this.oDoTopSpeed.el.style.fontSize = "16.9px";
    this.oDoTopSpeed.el.style.fill = "gray";
    if (maxVal <= 100) {
      this.oDoMidSpeed.el.style.visibility = "hidden";
    } else if (maxVal <= 200) {
      this.oDoMidSpeed.el.textContent = Math.round(maxVal * 0.75);
      this.oDoMidSpeed.el.style.visibility = "";
    } else {
      var midVal = Math.round(maxVal * 0.5);
      this.oDoMidSpeed.el.textContent = midVal >= 1000 ? (midVal / 1000).toFixed(0) + "G" : midVal;
      this.oDoMidSpeed.el.style.visibility = "";
    }
  };
  ...
  openSpeedtestShow.prototype.LiveSpeed = function(data, Display) {
    ...
    if (Display === "speedToZero") {
      ...
      this.oDoLiveSpeed.el.textContent = ShowData;
      var sMax = this.scaleMax || 1000;
      this.oDoTopSpeed.el.textContent = sMax >= 1000 ? (sMax / 1000).toFixed(0) + "G+" : sMax + "+";
      this.oDoTopSpeed.el.style.fontSize = "16.9px";
      this.oDoTopSpeed.el.style.fill = "gray";
      return;
    }
  ...
  openSpeedtestGet.prototype.uRandom = function(size, callback) {
    var totalBytes = size * 1048576;
    var buffer = new Uint8Array(totalBytes);
    var chunkSize = 65536;
    for (var offset = 0; offset < totalBytes; offset += chunkSize) {
      crypto.getRandomValues(buffer.subarray(offset, Math.min(offset + chunkSize, totalBytes)));
    }
    Callback(callback);
  ...
  var openSpeedtestEngine = function() {
    ...
    function ShowIP() {
      if (requestIP) {
        Show.YourIP.el.textContent = vlkT('status.wait');
        ServerConnect(7);
        requestIP = false;
      }
      Show.ip();
    ...
    function runTasks() {
      ...
      if (OpenSpeedTestStart >= 0) {
        ...
        Show.showStatus(vlkT('status.autoStart'));
      ...
      if (openSpeedTestServerList === "fetch" && launch === true) {
        launch = false;
        Show.showStatus(vlkT('status.fetching'));
        ServerConnect(6);
      }
      if (launch === true) {
        if (SelectTest === "Ping") {
    ...
    function OpenSpeedtest() {
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "Download") {
          Show.showStatus(vlkT('status.init'));
          Get.reset();
          reSett();
          Show.reset();
        ...
        if (Status === "Downloading") {
          ...
          if (Startit == 0) {
            Startit = 1;
            Show.showStatus(vlkT('status.testingDown'));
          ...
          if (downloadTimeing >= dlDuration && ProG == "done") {
            if (SelectTest) {
              ...
              Show.showStatus(vlkT('status.done'));
              Show.Symbol(2);
            } else {
        ...
        if (Status === "Uploading") {
          if (Startit == 1) {
            Startit = 2;
            Show.showStatus(vlkT('status.testingUp'));
            currentSpeed = 0;
            Get.reset();
            Show.reset();
        ...
        if (Status === "Error") {
          Show.showStatus(vlkT('status.checkConn'));
          Show.ConnectionError();
          Status = "busy";
          clearInterval(Engine);
          var dummyElement = document.createElement("div");
          dummyElement.innerHTML = '<a xlink:href="/manual.html" style="cursor: pointer" target="_blank"></a>';
          var htmlAnchorElement = dummyElement.querySelector("a");
          Show.oDoLiveSpeed.el.textContent = vlkT('status.netError');
    ...
    function runEstimation() {
      Show.showStatus("Estimating link speed…");
      var estimBytes = 0;
      var estimReq = [];
      var numThreads = 2;
      var duration = 1500;
      var startTime = window.performance.now();
      function startEstimReq(idx) {
        var lastLoaded = 0;
        var req = new XMLHttpRequest();
        estimReq[idx] = req;
        req.open("GET", fianlPingServer.Download + "?n=" + Math.random(), true);
        req.responseType = "arraybuffer";
        req.onprogress = function(e) {
          var chunk = e.loaded - lastLoaded;
          if (chunk > 0) { estimBytes += chunk; }
          lastLoaded = e.loaded;
        };
        req.onload = function() {
          if (window.performance.now() - startTime < duration) {
            startEstimReq(idx);
          }
        };
      ...
      setTimeout(function() {
        for (var k = 0; k < estimReq.length; k++) {
          if (estimReq[k]) {
            try { estimReq[k].abort(); } catch (e) {}
            estimReq[k] = null;
          }
        }
